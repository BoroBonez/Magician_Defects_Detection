...
class Optitrack(Node):
  def __init__(self):
    ...
    self.declare_parameter('robot_id', 20)
    self.TRACKED_ROBOT_ID = self.get_parameter('robot_id').value
    self.declare_parameter('topic', '/optiTrack/poseSTL')

    self.publisher_ = self.create_publisher(PoseStamped, self.get_parameter('topic').value, qos.qos_profile_sensor_data)
  ...
  def receiveRigidBodyFrame(self, id, position, rotation):
    self.TRACKED_ROBOT_ID = self.get_parameter('robot_id').value
    if (id==self.TRACKED_ROBOT_ID):
      msg = PoseStamped()
      # retrieve as originally published by the optitrack
      msg.header.frame_id = "id: " + str(id)
