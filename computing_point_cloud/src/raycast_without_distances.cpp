...
private:
    void topic_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        //initialization
        if (first_iteration){
            probabilities.resize(F.rows(), -1.0f);
        ...
        for (std::size_t i = 0; i < cloudPC->points.size(); ++i) {
                ...
                if (hits.size() > 0) {
                        ...
                        probabilities[hits.front().id] =    ( probabilities[hits.front().id] * count[hits.front().id] + p_of_P_S(cloudPC->points[i], p) ) /
                                                            ( count[hits.front().id] + 1);
        ...
        float lowest_prob = min_element(probabilities);
        float highest_prob = max_element(probabilities); 

        for (std::size_t i = 0; i < augmentedcloud->points.size(); ++i) {

                int id = id_point_at_index[i];

                if (probabilities[id] - mean < 0) {
                        // yellow 255, 255, 0
                        // from yellow to red
                        augmentedcloud->points[i].r = 255;
                        augmentedcloud->points[i].g = ( 1 - (probabilities[id] - mean)/(lowest_prob - mean) ) * 255;
                        augmentedcloud->points[i].b = 0;

                } else {
                        // from yellow to green
                        augmentedcloud->points[i].r = ( 1 - (probabilities[id] - mean)/(highest_prob - mean) ) * 255;
    ...
    float min_element(const std::vector<float>& probabilities){
        float min = 100.0;
        for (std::size_t i = 0; i < probabilities.size(); ++i) {
            if (probabilities[i] > 0){
    ...
    float max_element(const std::vector<float>& probabilities){
        float max = -1.0;
        for (std::size_t i = 0; i < probabilities.size(); ++i) {
            if (probabilities[i] > 0){
